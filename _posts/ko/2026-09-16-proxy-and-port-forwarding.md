...
※ **SOCKS**는 "이 주소로 TCP 연결을 대신 맺어달라"고 부탁하는 규약입니다. HTTP
프록시처럼 안에 담긴 HTTP 요청을 들여다보지 않고, 그냥 TCP 연결만 대신 열어주기
때문에 HTTP가 아닌 트래픽도 그대로 통과시킵니다. 지금 널리 쓰이는 판은 SOCKS5로,
인증과 UDP, 그리고 도메인 이름을 그대로 넘기는 것(주소 풀이까지 프록시에 맡기는 것)을
지원합니다. `ssh -D`가 띄우는 프록시도 이 SOCKS5입니다.
{: .note}
...
### forward proxy와는 어떻게 다른가요?

**forward proxy**는 클라이언트가 존재를 알고 직접 지정해서 쓰는 중개자였죠.
`ssh -D`의 SOCKS 프록시가 그 예였습니다. **reverse proxy는 정반대입니다.**
클라이언트는 nginx가 있는지도 모르고, 지정하지도 않습니다. 그냥 주소 하나로
접속했을 뿐인데, 서버 운영자가 미리 세워둔 중개자가 알아서 뒤쪽 서버를 골라줍니다.

판별 기준은 위치가 아니라 **누구를 대신하는가**입니다.

- forward proxy — **클라이언트를 대신해** 목적지에 연결합니다. 클라이언트가 압니다.
- reverse proxy — **서버를 대신해** 요청을 받습니다. 클라이언트는 모릅니다.

`ssh -D`의 SOCKS 프록시 말고도 이런 것들이 forward proxy입니다.

- 회사나 학교 망의 HTTP 프록시 — 브라우저에 주소를 넣어두면 모든 요청이 그리로 갑니다
- Charles, mitmproxy처럼 내 트래픽을 들여다보려고 중간에 세우는 도구
...
앞서 본 두 그림을 겹쳐 보면 차이가 뚜렷합니다. **SOCKS 프록시는 `your machine`
안에** 있었고, **nginx는 `server side` 안에** 있습니다. 같은 "프록시"라는 말을
쓰지만 서 있는 편이 정반대인 거죠.
...
| 누구를 대신하나 | 대신하지 않음 | 클라이언트 (forward proxy) | 서버 (reverse proxy) |

세 가지를 한 줄로 줄이면 이렇게 됩니다.

- **NAT 포트 포워딩** — 정해진 곳으로만 갑니다. 클라이언트는 모릅니다. 프록시가 아닙니다.
- **Dynamic SSH 포워딩** — 내가 정한 forward proxy를 거쳐 아무 데나 갑니다. 클라이언트가 압니다.
