...
- 그림은 `viewBox` 로 그려지고 `max-width:100%` 로 넘칠 때만 줄어듭니다.
  가로 스크롤이 생길 수 없습니다. **억지로 늘리지는 않습니다**(`width:auto`) —
  그린 크기가 곧 글씨 크기라서, 좁고 긴 `flow` 그림을 본문 폭까지 늘리면 세로로 터집니다.
...
존 제목은 왼쪽에 붙고, 주석이 달린 상자에서 나가는 화살표는 주석 아래에서 시작합니다.
둘 다 세로 배치에서 화살표가 글자를 뚫고 지나가지 않게 하려는 것입니다.
